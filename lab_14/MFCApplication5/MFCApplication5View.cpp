...
void CMFCApplication5View::OnDraw(CDC* pDC) // CDC - контекст отображения для класса окна
{
   ...
   RGB(255,255,255) };      //белый 
	...
	newpen.CreatePen(PS_SOLID,		// стиль пера
					 6,				// ширина пера
					 dwColor[0]);	// цвет пера
	...
	CreatePen(PS_SOLID, 1, dwColor[0]);
	oldpen = pDC->SelectObject(&newpen);
	...
	newpen.CreatePen(PS_SOLID, 8, dwColor[0]);
	oldpen = pDC->SelectObject(&newpen);
...
#define PI 3.14
	...
	for (xcoord1 = 400; xcoord1 < 1000; xcoord1 += 2)
	{
		int x, y;
		x = xcoord1;
		y = (int)(sin((double)xcoord1 * PI / 180) * 70);
		// устанавливает пиксель в заданных координатах в заданном цвете
		pDC->SetPixel(xcoord1, y + 80, dwColor[3]);
	}
	...
	{
		pDC->SetPixel(x1, y1, dwColor[2]);
		newpen.CreatePen(PS_SOLID, 6, dwColor[2]);
		oldpen = pDC->SelectObject(&newpen);
		pDC->MoveTo(x1, y1);
		newpen.DeleteObject();
	...
	for (xcoord1 = 400; xcoord1 < 800; xcoord1 += 2)
	{
		...
		x = xcoord1;
		y = x + 3;
		pDC->SetPixel(xcoord1, y, dwColor[1]);
	}
	...
	polylpts[3].x = 10;
	polylpts[3].y = 30;
	pDC->Polyline(polylpts, 4); // массив точке и количество точек
